...
class NodesController extends Controller
{
    ...
    public function UpdateScore($proxy, Request $request)
    {
        ...
        for ($k = 0; $k < count($up); $k++) {
            for ($i = 0; $i < count($n); $i++) {
                for ($j = $i; $j < count($n); $j++) {
                    $x[$up[$k]][$n[$i]][$n[$j]] = $x[$up[$k]][$proxy][$n[$i]] * $x[$up[$k]][$proxy][$n[$j]];
                }
            }
        }
        ...
        for ($k = 0; $k < count($up); $k++) {
            for ($i = 0; $i < count($n); $i++) {
                for ($j = $i + 1; $j < count($n); $j++) {
                    echo $up[$k] . " - " . $n[$i] . " - " . $n[$j] . " = " . $x[$up[$k]][$n[$i]][$n[$j]] .     "<hr color='FFFF00'>";
                    //Atualiza o julgamento
                    Judments::where('id_node', $up[$k])->where('id_node1', $n[$i])->where('id_node2', $n[$j])->update(['score' => ($x[$up[$k]][$n[$i]][$n[$j]])]);
                }
            }
        }
        return redirect('/nodes');
